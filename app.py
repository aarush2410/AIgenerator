...
#Streamlit Code
choice = st.sidebar.selectbox("Select your choice", ["Home", "DALL-E", "Stable Diffusers"])

if choice == "Home":
    st.title("AI Image Generation App")
    with st.expander("About the App"):
        st.write("This is a simple image generation app that uses AI to generates images from text prompt. As of now DALL-E functionality is not working reason due bills.")
...
elif choice == "Stable Diffusers":
    st.subheader("Image generation using Stable Diffusers")
    input_prompt = st.text_input("Enter your text prompt")
    if input_prompt is not None:
        if st.button("Generate Image"):
            ...
            st.image(image_output, caption="Generated by Stable Diffusers")